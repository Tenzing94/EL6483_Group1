 ...
 */
 ...
 /*
  * THE STEPS THE ROBOT TAKES IN ORDER TO COMPLETE THE COURSE
  *   1. A variable named "frequency_bin_index" will be used by the robot to index into one of the Bins generated by FFT
  *   2. "frequency_bin_index" is initialized to 20, which refers to the Bin that represents 5KHz signal
  *   3. The robot will make a 360 degree turn to read the signals. It will then pick the signal and direction with the largest magnitude and start moving towards it
  *   4. Once that bin has been found, it will then look for the next signal. NOTE that once the robot has found, lets say 7k Beacon, it will only look for signals larger than 7k from that point on.
  *   5. This way, the robot can NEVER go from a higher beacon to a lower one.
  *   6. Once "frequency_bin_index" is 38, meaning the robot has found the direction of 9.5k signal, it will move towards it. Once it reaches the beacon, it will halt the program.
  *   
  * MISC
  *   1. There is a red LED that is on the robot. Everytime it blinks, it means that the robot has found another beacon. 
  *      When the robot stops (after finding 9.5K beacon), the LED will light up permanently, indicating that it has found the last beacon.
  *   2. There is a small LCD next to the red LED. This LCD displays the current frequency it is following, and the value of the bin (magnitude of the signal).
  *   
  */
...
#define ULTRASONIC_STOP_DISTANCE_CM 15 // Distance at which the robot stops after detecting an object in front of it.
 ...
 *   16           ---    225
 */
#define SIZE_OF_ARRAY 8 // Number of times that the robot stops in order to take a reading. 
#define CIRCLE_DELAY_IN_MS 310 
...
// Makes a maneuver to avoid a beacon.
...
void sampleData()
{
  ...
  //threePointMovingAverage(); // We decided not to use the Three Point Moving Average filter
  
  // display current freq and mag on LCD Screen
...
void scan_full_circle()
{
  ...
  { 
    ...
    for (int j = frequency_bin_index; j < 39; j++) // Only consider the current frequency and higher.
    {
